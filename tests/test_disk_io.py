...
def test_disk_io_csv_save_success(io, tmp_path):
    ...
    io.csv_rows.append(["118600000", "AM", "Dublin airport Tower", "O"])
    io.csv_save(str(file_path), ",")
    io.csv_rows = []
    io.csv_load(csv_file=str(file_path), delimiter=",")
    assert io.csv_rows == [["118600000", "AM", "Dublin airport Tower", "O"]]
